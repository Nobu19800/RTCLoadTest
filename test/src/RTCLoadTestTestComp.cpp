...
{
  RTCLoadTestTestInit(manager);
  RTCLoadTestInit(manager);
  RTC::RtcBase* comp;

  // Create a component
  comp = manager->createComponent("RTCLoadTestTest");
...
bool RunTest()
{
  ...
  comp = mamager.getComponent("RTCLoadTestTest0");
  if (comp == nullptr)
  {
  ...
  RTCLoadTestTest* testcomp = dynamic_cast<RTCLoadTestTest*>(comp);
  if (testcomp == nullptr)
  {
    std::cerr << "Component get failed." << std::endl;
    return false;
